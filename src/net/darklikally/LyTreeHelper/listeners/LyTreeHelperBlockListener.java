    ...
    public void destroyTree(Player player, Block firstBlock) {
        ...
        if(!worldConfig.isDestroyAll() && worldConfig.isDestroyAllWood()) {
            ...
            if(detectTreeWoodLeaves(firstBlock, stdRetVal, worldConfig, processNumber)) {
                ...
                // iConomy stuff
                if(worldConfig.isiConomySupport() && this.plugin.getEconomy() != null) {
                    Method economy = this.plugin.getEconomy();
                    if(player != null) {
                        ...
                        if(account != null) {
                            if(account.hasEnough(worldConfig.getiConomyMoneyOnFullDest())) {
                                account.subtract(worldConfig.getiConomyMoneyOnFullDest());
                            } else {
    ...
    public void onBlockBreak(BlockBreakEvent event) {
        ...
        if(event.getBlock().getType() == Material.LEAVES) {
            if(worldConfig.isDestroyAll()) {
                if(!this.plugin.getPermissions().hasPermission(event.getPlayer(), "destroyall")) {
                    return;
                }
